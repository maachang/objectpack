...
public final class Json {
	...
	private static final void _encode(StringBuilder buf, Object base, Object target) throws IOException {
	...
	private static final void encodeJsonMap(StringBuilder buf, Object base, Map map) throws IOException {
		boolean flg = false;
		Map mp = (Map) map;
	...
	private static final void encodeJsonList(StringBuilder buf, Object base, List list) throws IOException {
		boolean flg = false;
		List lst = (List) list;
		buf.append("[");
		int len = lst.size();
	...
	private static final void encodeJsonArray(StringBuilder buf, Object base, Object list) throws IOException {
		boolean flg = false;
	...
	/** [decodeJSON]１つの要素を変換. **/
	private static final Object decJsonValue(int[] n, int no, String json) throws IOException {
		int len;
	...
	/** JSON_Token_解析処理 **/
	private static final List<Object> analysisJsonToken(String json) throws IOException {
		int s = -1;
		char c;
		int cote = -1;
		int bef = -1;
	...
	private static final Object createJsonInfo(int[] n, List<Object> token, int type, int no, int len)
		...
		// map.
			...
			ret = new AndroidMap<String, Object>();
			String key = null;
			for (int i = no + 1; i < len; i++) {
				value = (String) token.get(i);
				if (":".equals(value)) {
	...
	private static final SimpleDateFormat _getISO8601() {
		...
		if (n == null || n.get() == null) {
			n = new WeakReference<SimpleDateFormat>(new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssXXX"));
			iso8601.set(n);
		}
		return n.get();
	}
