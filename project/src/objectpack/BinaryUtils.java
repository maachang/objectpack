...
public class BinaryUtils {
	...
	public static final boolean eqEng(byte[] src, byte[] dest) {
		...
		byte s, d;
		for (int i = 0; i < len; i++) {
			if ((s = src[i]) == (d = dest[i]) || (s >= (byte) 'A' && s <= (byte) 'z' && d >= (byte) 'A'
					&& d <= (byte) 'z'
					&& (s + SMALL_BIG_CODE == d || s - SMALL_BIG_CODE == d || s == d + SMALL_BIG_CODE
							|| s == d - SMALL_BIG_CODE || s + SMALL_BIG_CODE == d + SMALL_BIG_CODE
							|| s - SMALL_BIG_CODE == d + SMALL_BIG_CODE || s + SMALL_BIG_CODE == d - SMALL_BIG_CODE
	...
	public static final boolean eqEng(byte[] src, int off, int len, byte[] dest) {
		...
		byte s, d;
		for (int i = 0; i < len; i++) {
			if ((s = src[i + off]) == (d = dest[i]) || (s >= (byte) 'A' && s <= (byte) 'z' && d >= (byte) 'A'
					&& d <= (byte) 'z'
					&& (s + SMALL_BIG_CODE == d || s - SMALL_BIG_CODE == d || s == d + SMALL_BIG_CODE
							|| s == d - SMALL_BIG_CODE || s + SMALL_BIG_CODE == d + SMALL_BIG_CODE
							|| s - SMALL_BIG_CODE == d + SMALL_BIG_CODE || s + SMALL_BIG_CODE == d - SMALL_BIG_CODE
	...
	public static final boolean oneEng(byte s, byte d) {
		return (s == d || (s >= (byte) 'A' && s <= (byte) 'z' && d >= (byte) 'A' && d <= (byte) 'z'
				&& (s + SMALL_BIG_CODE == d || s - SMALL_BIG_CODE == d || s == d + SMALL_BIG_CODE
						|| s == d - SMALL_BIG_CODE || s + SMALL_BIG_CODE == d + SMALL_BIG_CODE
						|| s - SMALL_BIG_CODE == d + SMALL_BIG_CODE || s + SMALL_BIG_CODE == d - SMALL_BIG_CODE
						|| s - SMALL_BIG_CODE == d - SMALL_BIG_CODE)));
	...
	public static final int indexOf(final byte[] buf, final byte[] chk, final int off, int vLen) {
		...
		// 複数バイナリ検索.
			...
			for (int i = off; i < vLen; i++) {
				...
				if (i < vLen) {
					for (next = i + len, j = i + 1, k = 1; j < next && buf[j] == chk[k]; j++, k++)
						;
					if (j == next) {
						return i;
					}
	...
	public static final int lastIndexOf(final byte[] buf, final byte[] chk, int off) {
		...
		// 複数バイナリ検索.
			...
			for (int i = off; i >= 0; i--) {
				...
				if (i >= len) {
					for (next = i - (len + 1), j = i - 1, k = len - 1; j > next && buf[j] == chk[k]; j--, k--)
						;
					if (j == next) {
						return j + 1;
					}
	...
	public static final int indexOfEng(final byte[] buf, final byte[] chk, final int off, int vLen) {
		...
		// 複数バイナリ検索.
			...
			for (int i = off; i < vLen; i++) {
				...
				if (i < vLen) {
					for (next = i + len, j = i + 1, k = 1; j < next && oneEng(buf[j], chk[k]); j++, k++)
						;
					if (j == next) {
						return i;
					}
	...
	public static final int lastIndexOfEng(final byte[] buf, final byte[] chk, int off) {
		...
		// 複数バイナリ検索.
			...
			for (int i = off; i >= 0; i--) {
				...
				if (i >= len) {
					for (next = i - (len + 1), j = i - 1, k = len - 1; j > next && oneEng(buf[j], chk[k]); j--, k--)
						;
					if (j == next) {
						return j + 1;
					}
