...
public class JSnappy {
     ...
     */
    public static final JSnappyBuffer compress(byte[] in, JSnappyBuffer out)
            throws Exception {
     ...
     * @return compressed data block
     */
    public static final JSnappyBuffer compress(byte[] in, int offset, int length)
            throws Exception {
    ...
    /**
     * Equivalent to
     * <code>compress(in.getData(), 0, in.getLength(), null)</code>.
     * 
     * @param in
     *            data to be compressed
     * @return compressed data block
     */
    public static final JSnappyBuffer compress(JSnappyBuffer in)
            throws Exception {
     ...
     */
    public static final JSnappyBuffer compress(JSnappyBuffer in,
            JSnappyBuffer out) throws Exception {
     ...
     * @return reference to <code>out</code>
     */
    public static final JSnappyBuffer decompress(byte[] in, JSnappyBuffer out)
            throws Exception {
     ...
     * @return decompressed data block
     */
    public static final JSnappyBuffer decompress(byte[] in, int offset,
    ...
    /**
     * Equivalent to
     * <code>decompress(in.getData(), 0, in.getLength(), null)</code>.
     * 
     * @param in
     *            compressed data block
     * @return decompressed data block
     */
    public static final JSnappyBuffer decompress(JSnappyBuffer in)
            throws Exception {
    ...
    /**
     * Equivalent to
     * <code>decompress(in.getData(), 0, in.getLength(), out)</code>.
     * 
     * @param in
     *            compressed data block
     * @param out
     ...
     * @exception 例外.
     */
    public static final JSnappyBuffer compress(byte[] in, int offset,
            int length, JSnappyBuffer out, int shift) throws Exception {
        int lenM4 = length - 4;
        ...
        // 圧縮バッファの調整.
        if (out == null) {
            out = new JSnappyBuffer(calcMaxCompressLength(length));
        } else {
        ...
        int n = length;
        while (n > 0) {
            target[targetIndex++] = (n >= 128) ? (byte) (0x80 | (n & 0x7f))
                    : (byte) n;
            n >>= 7;
        }
        ...
        for (int i = offset; i < iLen; i++) {
            _cc[(((((in[i] & 0xff) << 24) | ((in[i + 1] & 0xff) << 16)
                    | ((in[i + 2] & 0xff) << 8) | (in[i + 3] & 0xff)) * HASH) >> hashShift)
                    & _msk] = i;
        }
        ...
        for (int i = offset + 4; i < offLenM4; i++) {
            ...
            // 圧縮条件が存在する場合.
            if ((n = ((in[i] & 0xff) << 24) | ((in[i + 1] & 0xff) << 16)
                    | ((in[i + 2] & 0xff) << 8) | (in[i + 3] & 0xff)) == (((in[fp = _cc[((n * HASH) >> hashShift)
                    & _msk]] & 0xff) << 24)
                ...
                if (in[fp + 4] == in[i + 4]) {
                    ...
                    // 圧縮対象同一までチェック.
                    for (tLen = (io + LIMIT < offLen) ? ((o + LIMIT < i) ? LIMIT
                            : i - o)
                            : offLen - io; hLen < tLen && in[o++] == in[io++]; hLen++)
                        ;
                } else {
                    hLen = 4; // 圧縮連続長.
                }
     ...
     * @exception 例外.
     */
    public static final JSnappyBuffer decompress(final byte[] in,
            final int offset, final int length, JSnappyBuffer out)
            throws Exception {
        int p;
        ...
        while (sourceIndex < offLen && targetIndex < targetLength) {
            ...
            if ((bc = in[sourceIndex] & 3) == 0) {
                ...
                if ((o = (n = (in[sourceIndex++] >> 2) & 0x3f) - 60) > -1) {
                    for (o++, c = 1, n = (in[sourceIndex] & 0xff); c < o; n |= (in[sourceIndex
                            + c] & 0xff) << (c * 8), c++)
                        ;
                    sourceIndex += o;
                }
            ...
            // 圧縮情報の取得.
                ...
                o = ((in[sourceIndex] & 0xe0) << 3)
                        | (in[sourceIndex + 1] & 0xff);
                sourceIndex += 2;
            } else if (bc == 2) {// 2.
                n = ((in[sourceIndex] >> 2) & 0x3f) + 1;
                o = (in[sourceIndex + 1] & 0xff)
                        | ((in[sourceIndex + 2] & 0xff) << 8);
                sourceIndex += 3;
            } else { // 3.
                n = ((in[sourceIndex] >> 2) & 0x3f) + 1;
                o = (in[sourceIndex + 1] & 0xff)
                        | ((in[sourceIndex + 2] & 0xff) << 8)
                        | ((in[sourceIndex + 3] & 0xff) << 16)
                        | ((in[sourceIndex + 4] & 0xff) << 24);
                sourceIndex += 5;
            }
            // 圧縮情報のセット.
        ...
        if (targetIndex > targetLength) {
            throw new IOException(
                    "Superfluous input data encountered on offset (index:"
