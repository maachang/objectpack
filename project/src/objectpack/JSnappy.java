...
public class JSnappy {
	 ...
	 * 
	 * @param in  data to be compressed
	 * @param out JSnappyBuffer for compressed data block
	 ...
	 * 
	 * @param in  compressed data block
	 * @param out JSnappyBuffer for decompressed data block
	 * @return reference to <code>out</code>
	 */
	 ...
	 * snappy圧縮.
	 * 
	 * @param in     圧縮対象のバイナリを設定します.
	 * @param offset 圧縮対象バイナリの開始位置を設定します.
	 * @param length 圧縮対象の長さを設定します.
	 * @param out    圧縮結果を格納するBufferオブジェクトを設定します. 設定されない場合は、内部で新規作成されます.
	 * @param shift  圧縮テーブル長を増やす場合は、この値に整数を設定します.
	 * @return JSnappyBuffer 圧縮されたBufferオブジェクトが返却されます. この情報は第４引数でオブジェクトを渡した場合、
	 *         同様オブジェクトが返却されます.
	 * @exception 例外.
	 */
	public static final JSnappyBuffer compress(byte[] in, int offset, int length, JSnappyBuffer out, int shift)
			throws Exception {
		int lenM4 = length - 4;
	 ...
	 * snappy解凍.
	 * 
	 * @param in     解凍対象のバイナリを設定します.
	 * @param offset 解凍対象バイナリの開始位置を設定します.
	 * @param length 解凍対象の長さを設定します.
	 * @param out    解凍結果を格納するBufferオブジェクトを設定します. 設定されない場合は、内部で新規作成されます.
	 * @return JSnappyBuffer 解凍されたBufferオブジェクトが返却されます. この情報は第４引数でオブジェクトを渡した場合、
	 *         同様オブジェクトが返却されます.
	 * @exception 例外.
	 */
	public static final JSnappyBuffer decompress(final byte[] in, final int offset, final int length, JSnappyBuffer out)
			throws Exception {
		int p;
		...
		if (targetIndex > targetLength) {
			throw new IOException("Superfluous input data encountered on offset (index:" + targetIndex + " max:"
					+ targetLength + ")");
		}
		return out;
	}
