...
class FastMethodElementsNormal extends FastMethodElements {
    /** メソッド情報格納用 **/
    private static class FastMethodEmtNormal {
        ...
        /** コンストラクタ. **/
        protected FastMethodEmtNormal(Method method)
     ...
     * コンストラクタ.
     */
    protected FastMethodElementsNormal() {
    }
     ...
     *                例外.
     */
    public FastMethodElementsNormal(Class clazz) throws NoSuchMethodException,
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (Modifier.isPublic(m.getModifiers())) {
                    ...
                    if (map.containsKey(name)) {
                        FastMethodEmtNormal f = map.get(name);
                        if (f.next == null) {
                        ...
                        map.put(name, new FastMethodEmtNormal(m));
                    }
                }
            }
        }
    ...
    public boolean invoke(Object[] result, Object target, String name,
        ...
        FastMethodEmtNormal emt = map.get(name);
        if (argsLen == 0) {
            while (emt != null) {
